...
write.csv(output1, file = "exp_ANOVA_table.csv", sep = ",")

exp_Tukey_gene <-  TukeyHSD(exp_ANOVA_1, "gene", ordered = TRUE)
exp_Tukey_gene_table <- as.data.frame(exp_Tukey_gene$gene)

write.csv(exp_Tukey_gene_table, file = "exp_Tukey_gene_table.csv", sep = ",")
...
write.csv(exp_Tukey_treatment_table, file = "exp_Tukey_treatment_table.csv", sep = ",")
